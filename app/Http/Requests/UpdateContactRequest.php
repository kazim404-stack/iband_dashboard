...
class UpdateContactRequest extends FormRequest
{
    ...
    {
        $contactId = $this->route('contact'); // For PUT/PATCH update
    ...
    public function messages(): array
    {
        return [
            ...
            'state.array'        => 'State must be a valid array.',
            'state.*.required'   => 'Each state field is required.',
            'state.*.string'     => 'Each state must be valid text.',
            'state.*.max'        => 'Each state may not exceed 255 characters.',

            'address.required'   => 'Address is required.',
            'address.array'      => 'Address must be a valid array.',
            'address.*.required' => 'Each address field is required.',
            'address.*.string'   => 'Each address must be valid text.',
